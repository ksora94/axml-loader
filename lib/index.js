...
function _default(content) {
  ...
  (0, _parser["default"])(content).then(function (parts) {
    if (parts.config) {
      output += getRequirePath('config', ["file-loader?name=[name].json&outputPath=".concat(outputPath)], parts.config.attrs.src);
    }

    if (parts.template) {
      output += getRequirePath('template', ["file-loader?name=[name].axml&outputPath=".concat(outputPath)], parts.template.attrs.src);
    }

    if (parts.style) {
      output += getRequirePath('style', ["file-loader?name=[name].acss&outputPath=".concat(outputPath)].concat(_toConsumableArray(getLangLoader(parts.style.attrs.lang)), ["".concat(__dirname, "/transform/style.js")]), parts.style.attrs.src);
    }
